<!-- This page displays the customer's previous orders. -->
<?php
// As always we require the session start and other things from the base file.
  require 'reso/base.php';
// We check that the user is logged in so that we can pull their previous orders.
  if(isset($_SESSION['logged_in']) && $_SESSION['logged_in'] == true) {
    $userID = $_SESSION['user']['user_id'];
// Use the standard header for all our main pages.
    require 'reso/frames/header.php';
    ?>
    <body>
    <?php
// Input the navbar on our page.
    require 'reso/frames/navbar.php';
    ?>
    <div class="container">
    <?php
    // We get orders ids. The query is in descending order by date of order by convention.
    ...
    if (mysqli_num_rows($orders) > 0) {
      ...
      // In the case that we retrieve no orders, inform the customer that they have no previous orders.
    } else {
  ...
  <?php require 'reso/frames/scripts.php'; ?>
    ...
    <?php
    // If the user is not logged in, redirect to the log-in page and give the previous location of orders.
  } else {
